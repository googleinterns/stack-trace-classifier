...
if __name__ == "__main__":
  ...
  arg_parser.add_argument(
      ...
      required=True,
      help=
      'configuration file path, expected to be in the format as outlined in config.proto'
  )
  arg_parser.add_argument(
      '--bigquery_config',
      nargs=1,
      type=str,
      help=
      'bigquery configuration file path to pass in expected to be in format of big_query_config.proto'
  )
  ...
  if args.bigquery_config:
    ...
    # BigQuery Schematics
    df = get_input_dataframe_table(bigquery_config.project_id,
                                   bigquery_config.dataset_id,
                                   bigquery_config.input_table_id)
    ...
    output_dataframe_to_gbq(output_df, bigquery_config.project_id,
                            bigquery_config.dataset_id,
                            bigquery_config.output_table_id)
