...
class KMeansClustererTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.simple_dataframe = pd.read_json(
        'testdata/k_means_clusterer/simple_data.json', orient='columns')
    self.repeated_dataframe = pd.read_json(
        'testdata/k_means_clusterer/repeated_data.json', orient='columns')
    self.stack_trace_dataframe = pd.read_json(
        'testdata/k_means_clusterer/stack_trace_data.json', orient='columns')
    super(KMeansClustererTest, self).setUp()

  def test_cluster_errors_simple(self):
    """Simple test on what should be 2 different clusters."""
    clusterer = KMeansClusterer(self.simple_dataframe,
                                self.config_human_readable)
    clusterer.cluster_errors()

    # number of clusters should be 2
    self.assertEqual(len(clusterer.df['clusterer_output'].unique()), 2)

  def test_cluster_errors_repeated(self):
    """Test with repeated data, should still only be 2 clusters despite repeated data."""
    clusterer = KMeansClusterer(self.repeated_dataframe,
                                self.config_human_readable)
    clusterer.cluster_errors()
  ...
  def test_cluster_errors_stack_trace(self):
    ...
    clusterer = KMeansClusterer(self.stack_trace_dataframe,
                                self.config_stack_trace_lines)
    clusterer.cluster_errors()
