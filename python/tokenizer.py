...
class Tokenizer:
    ...
    def stack_trace_line_tokenizer(self, input_string):
        """
        Tokenization method for parsing the input_string into a list of stack trace strings, each
        token representing a line in the stack trace.
        ...
        for line in input_string_lines:
            if line.startswith('\tat'):
                # we only care about the "class" or anything after the \tat and before line number
                stack_lines.append(line[4:line.index('(')])

        # filter out minimum length tokens
        stack_lines = list(filter(lambda line: len(line) >= self.min_token_len, stack_lines))
