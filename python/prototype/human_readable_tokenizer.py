...
def error_tokenize(trace):
  """A tokenizer for java stack-traces.

  First uses processStackTrace to extract
  human readable errors from the stack trace then tokenizes the human readable error
  ...
  tokens = list(filter(lambda w: not re.fullmatch(r'[0-9a-f|:|\.]+', w),
                       tokens))
  # Removes empty strings
  tokens = list(filter(lambda w: w, tokens))
  # Removes all words not in the "english"
  # dictionary this one is a little bit extreme ..
